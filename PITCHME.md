...
### 2次のAdams-Bashforth法
$x_k$と $ x _{k+1}$の間の$f(x, y(x))$を現在いる第$k$ステップの情報$x _k,\ f _k( = f(x _k, y _k))$と, １つ前の第k-1ステップの情報$x _{k-1}, f _{k-1}( = f(x _{k-1}, y _{k-1}))$を使った１次のLagrange補間多項式 $g _1(x)$で近似することを考える.
